...
def start(config):
    ...
    async def handler(event):
        for output_channel in output_channel_entities:
            ...
            #Get channel name
            send_channel_id = event.message.peer_id.channel_id
            index_of_channel = config["input_channel_ids"].index(int(send_channel_id))
            try:
            ...
            if (event.photo):
                try:
                    print('File Name :' + str(event.file.name))
                    path = await client.download_media(event.media, "image/")
